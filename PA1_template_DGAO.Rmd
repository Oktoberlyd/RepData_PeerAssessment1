...
### 1. Load the data
```{r}
 download.file("https://d396qusza40orc.cloudfront.net/repdata%2Fdata%2Factivity.zip", "C:/Users/dgao/activity_data.zip")

activity_monitoring_data <- read.csv(unzip("C:/Users/dgao/activity_data.zip"))
```
...
#TJEK
#activity_monitoring_data$datetime <- ymd_hm(paste(activity_monitoring_data$date, activity_monitoring_data$hour, #activity_monitoring_data$minute), sep = "")
```
...
Mean total steps per day is `r mean_total_steps_per_day` and median total steps is `r median_total_steps_per_day`
...
The interval with the highest number of average steps is named `r max_interval`
...
After imputation mean total steps per day is `r mean_total_steps_per_day_imputed` and median total steps is `r median_total_steps_per_day_imputed`

There is a slight deviation in the mean and median after imputation. After imputation the mean differs with `r mean_total_steps_per_day - mean_total_steps_per_day_imputed` and the median differs with `r median_total_steps_per_day - median_total_steps_per_day_imputed`
